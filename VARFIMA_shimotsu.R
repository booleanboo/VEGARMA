...
library(msos)
  ...
  xt<-stats::filter(at[,1],theta1,method="convolution",sides=1)
  yt<-stats::filter(at[,2],theta2,method="convolution",sides=1)
  ...
  varfima.whittle<-function(d1,d2)
  {
    ...
    wx <- 1/sqrt(2 * pi * n) * xt[2000:2999] %*% weight.mat #1*56
    wy <- 1/sqrt(2 * pi * n) * yt[2000:2999] %*% weight.mat  #1*56
    
    
    for(j in 1:m)
    {
      
      Lamdaj<-matrix(c(lambdaj[j]^(-d1)*exp(1i*(pi-lambdaj[j])*d1/2),0,0,lambdaj[j]^(-d2)*exp(1i*(pi-lambdaj[j])*d2/2)),2,2)
